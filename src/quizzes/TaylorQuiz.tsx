...
export function TaylorQuiz({ onBack }: { onBack: () => void }) {
  ...
  const [finalResult, setFinalResult] = useState<QuizResult | null>(() => {
    const params = new URLSearchParams(window.location.search);
  ...
  const calculateResult = (finalScores: Record<string, number>) => {
    let topId = Object.keys(results)[0]; // default
    let maxScore = -1;
    ...
    >
      ...
      <div className="relative z-10">
        ...
        <p className="text-lg text-slate-500 mb-10 max-w-md mx-auto leading-relaxed">
          Are you a poetic folklore soul, a chaotic reputation baddie, or a glittery 1989 pop icon? Find your true era.
        </p>
...
function ResultScreen({
  ...
  const handleShare = async () => {
    const shareUrl = `${window.location.origin}${window.location.pathname}?quiz=taylor`;
    const shareText = `I got the ${result.name} Era! 🪩 What's your Taylor Swift era? Take the quiz: ${shareUrl}`;
  ...
  return (
    ...
    >
      <div className={`absolute -top-20 -right-20 w-64 h-64 rounded-full blur-3xl opacity-20 ${result.color.split(" ")[0]}`} />
      <div className={`absolute -bottom-20 -left-20 w-64 h-64 rounded-full blur-3xl opacity-20 ${result.color.split(" ")[0]}`} />
      ...
      <div className="relative z-10">
        ...
        >
          <img src={result.imageUrl} alt={result.name} referrerPolicy="no-referrer" className="w-full h-full object-cover drop-shadow-lg" />
